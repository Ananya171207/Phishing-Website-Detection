...
def _same_domain(target: str, base_registered: str):
    try:
        ...
        return _domain_parts(host)[3] == base_registered
    except Exception:
        return False
...
def extract_features(raw_url: str, fetch_page: bool = True):
    ...
    f.update({
        ...
        'https_token': int('https' in host.replace('https://','') or 'https' in path_and_query.lower()),
...
def _domain_features(f, registered, warnings):
    ...
    if whois and registered:
        try:
            ...
            warnings.append('WHOIS details were unavailable; domain-age features use neutral values.')
    # These originally depend on third-party ranking/search services. Neutral defaults avoid hidden API keys.
    f['web_traffic'] = 0
    f['google_index'] = 0
    f['page_rank'] = 0
